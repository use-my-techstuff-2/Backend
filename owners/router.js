...
router.get('/', (req, res) => {
    ...
    .catch(err => res.status(500).json({ error: "Could not retrieve the owners from the database" }))
})
...
router.get('/:id', (req, res) => {
    ...
    .then(owner => {
        if(owner) {
            ...
            .catch(err => res.status(500).json({ error: "Error retrieving gadgets for that owner "}))
        } else {
...
router.put('/:id', (req, res) => {
    ...
    .update(changes).then(count => {
        if(count) {
            ...
            .catch(err => res.status(404).json({ message: "Could not find owner with the given ID" }))
        } else {
            res.status(404).json({ message: "Could not find owner with the given ID" })
        }
    })
    .catch(err => {
